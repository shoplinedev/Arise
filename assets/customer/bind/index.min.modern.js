(() => {
    ...
    (() => {
        ...
        const bind_sendBindPhoneVerificationCode = params => request_udbRequest.get("/udb/aq/mobile/bind/sendSms.do", {
            params
        });
        const sendBindEmailVerificationCode = params => request_udbRequest.get("/udb/aq/email/bind/sendCode.do", {
            params
        });
        const updateBindInfo = type => request_request.get(`/user/front/userinfo/sync/${type}`);
        const verifyBindPhoneVerificationCode = params => request_udbRequest.get("/udb/aq/mobile/bind/bind.do", {
            params
        });
        const verifyBindEmailVerificationCode = params => request_udbRequest.get("/udb/aq/email/bind/bind.do", {
            params
        });
        ...
        const PHONE = "phone";
        class BindAccount extends base {
            constructor({id, form}) {
                super({
                    id,
            ...
            initForm() {
                ...
                this.bindForm = new commons_form({
                    id: this.formId,
                    fields,
            ...
            async sendVerifyCode() {
                const {mode} = this.form.queryParams;
                ...
                await wrapArmorCaptcha({
                    onSendCode: async captchaToken => {
                        ...
                        if (mode === PHONE) {
                            ...
                            res = await bind_sendBindPhoneVerificationCode(verifyCodePrarms);
                        } else {
                            const verifyCodePrarms = super.formatRequestBody({
                                ...params,
                                email: acct
            ...
            async onSubmit(data = {}) {
                const {mode} = this.form.queryParams;
                const params = this.form.UDBParams;
                if (mode === PHONE) await verifyBindPhoneVerificationCode(super.formatRequestBody({
                    ...params,
                    mobile: data[mode],
                    smscode: data.verifycode
                })); else await verifyBindEmailVerificationCode(super.formatRequestBody({
                    ...params,
                    email: data[mode],
                    emailcode: data.verifycode
                }));
                await updateBindInfo("phone" === mode ? "2" : "1");
        ...
        const account = BindAccount;
        class Bind extends customer {
            constructor({id = "customer-bind"}) {
                super({
                    id,
                    formType: "bind"
                });
            ...
            async sendVerifyCode() {
                const {type, mode} = this.queryParams;
                const params = this.UDBParams;
                if ("member" === type && [ "email", "phone" ].includes(mode)) await wrapArmorCaptcha({
                    onSendCode: async captchaToken => {
                        const {stoken: lastStoken} = await sendUniversalVerificationCode({
                            ...params,
                            captcha: captchaToken
                        });
                        super.updateToken(params, {
            ...
            async onSubmit(formValue = {}) {
                const {type, mode} = this.queryParams;
                const params = this.UDBParams;
                if ("member" === type && [ "email", "phone" ].includes(mode)) {
