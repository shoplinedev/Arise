(() => {
    ...
    (() => {
        ...
        const loginAccount = params => request_udbRequest.get("/udb/lgn/login/verify.do", {
            params
        });
        const signInThird = params => request_udbRequest.get("/udb/lgn/third/open/login.do", {
            params
        });
        ...
        function uniqueObjectArray(arr, prop, callback) {
            return arr.filter(((item, index) => {
                let result = true;
                if ("function" === typeof callback) result = callback(item, index, arr);
        ...
        class ThirdPartLogin {
            ...
            formatThirdPartUrls() {
                ...
                const filterByName = ({name}) => types.includes(name);
                return uniqueObjectArray(thirdLoginConfigVos, "name", filterByName).map((({name, appId}) => ({
                    type: name,
                    url: (0, url.stringifyUrl)(THIRD_LOGIN_HREF[name], {
                        response_type: "code",
                        client_id: appId,
                        state: {
                            appid: SLAppid,
                        ...
                        ..."facebook" === name && platformChannelId && {
            ...
            saveThirdChannelInfo(state, {isFirst, thirdNickName}) {
                let stateObject = {};
                let method = "";
                try {
                ...
                if (stateObject && stateObject.channel) method = CHANNEL_TO_METHOD[stateObject && stateObject.channel];
                if (isFirst) this.$$reports && this.$$reports.thirdReportSignUpSuccess && this.$$reports.thirdReportSignUpSuccess(this.form.eid, method);
                return {
        ...
        const third_part_login = ThirdPartLogin;
        ...
        const login_modal_defaultOptions = {
            modalId: MODAL_ID
        };
        class LoginModal {
            constructor(options = {}) {
                this.options = {
                    ...login_modal_defaultOptions,
                    ...options
                };
                ...
                if (!window.__SL_$__) window.__SL_$__ = window.jQuery;
                reportPageView();
                return this.init();
            }
            async init() {
                try {
                    this.loginInstance = new sign_in({
                        id: `${this.modalId}-signIn` || LOGIN_ID,
                        isModal: true,
                        success: () => this.refresh()
                    });
                    this.registerInstance = new sign_up({
            ...
            bindEvents() {
                ...
                if (!isMobile) return;
                const $modal = __SL_$__("#login-modal-container");
        ...
        const login_modal = () => new LoginModal({});
        __SL_$__((() => login_modal()));
    })();
})();