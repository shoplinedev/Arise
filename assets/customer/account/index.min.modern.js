(() => {
    ...
    (() => {
        ...
        const signOut = params => request_udbRequest.get("/udb/lgn/login/logout.do", {
            params
        });
        const HOME = "/";
        ...
        class Modal {
            constructor({modalId = ""} = {}) {
                this.modalId = `${common_DEFAULT_MODAL_ID_PRE}${modalId}`;
            ...
            toggleMaskClassName() {
                if (this.maskClosable) this.$modal.find(`.${common_bem("mask")}`).toggleClass(common_maskClosableClass, this.visibleState === common_VISIBLE);
            }
            afterAnimation() {
        ...
        class Card {
            ...
            initCard() {
                if (this.editable) {
                    __SL_$__(this.editIconSelector).on("click", (() => {
                        this.onEdit && this.onEdit();
                        __SL_$__(this.detailEleSelector).hide();
                        __SL_$__(this.editEleSelector).show();
                    }));
                    __SL_$__(this.cancelBtnSelector).on("click", (() => {
                        this.onCancel && this.onCancel();
                        __SL_$__(this.detailEleSelector).show();
                        __SL_$__(this.editEleSelector).hide();
                    }));
                    __SL_$__(this.saveBtnSelector).on("click", (async () => {
                        __SL_$__(this.saveBtnSelector).addClass("disabled btn--loading");
                        try {
                        ...
                        __SL_$__(this.saveBtnSelector).removeClass("disabled btn--loading");
                    }));
                }
        ...
        const card = Card;
        const getStoreRegisterConfig = () => request_request.get("/user/front/store/config/register");
        ...
        const getSubscriptAuths = ({platform}) => request_request.get("/user/front/sub/authorize", {
            params: {
                platform
            }
        });
        ...
        const reportInputSubscribeNewPhone = debounce_default()((() => {
            reportCenterEvent({
                module: report_Module.userCenter.subscribe,
                component: 145,
                action_type: report_ActionType.input,
        ...
        const verification_code_sendPhoneVerificationCode = params => request_udbRequest.get("/udb/aq/uni/sendSms.do", {
            params
        });
        const verification_code_sendEmailVerificationCode = params => request_udbRequest.get("/udb/aq/uni/sendEmailCode.do", {
            params
        });
        const verification_code_verifyPhoneVerificationCode = params => request_udbRequest.get("/udb/aq/uni/verifySms.do", {
            params
        });
        const verification_code_verifyEmailVerificationCode = params => request_udbRequest.get("/udb/aq/uni/verifyEmailCode.do", {
            params
        });
        const sendUniversalVerificationCode = params => {
            const sendCodeFn = params._method && params._method.includes("sms") ? verification_code_sendPhoneVerificationCode : verification_code_sendEmailVerificationCode;
            return sendCodeFn(helpers_formatRequestBody(params));
        };
        const verifyUniversalVerificationCode = (params, data) => {
            if (params.stoken) {
                const verifyCodeFn = params._method && params._method.includes("sms") ? verification_code_verifyPhoneVerificationCode : verification_code_verifyEmailVerificationCode;
                return verifyCodeFn(helpers_formatRequestBody({
                    ...params,
                    code: data.code
                }));
            }
        ...
        const countriesDialCodeMap = countries.reduce(((res, {iso2, dialCode}) => {
            res[dialCode] = iso2;
            return res;
        }), {});
        ...
        const getCurrentTime = () => request_request.get("/user/front/userinfo/getCurrentTime");
        ...
        const delete_account_BUTTON_LOADING_CLASS = "btn--loading";
        const PHONE = "phone";
        const TIMEOUT_CODE = "CCU0005";
        class DeleteAccount extends customer {
            constructor({id = "delete-account", containerId, mode, onClose}) {
                super({
                    id,
                ...
                this.$verify = __SL_$__(`#${containerId} .${id}__verify`);
                this.$notice = __SL_$__(`#${containerId} .${id}__notice`);
                this.$tips = null;
                this.bindEvents();
            }
            init() {
            ...
            bindEvents() {
                __SL_$__(`#${this.containerId} .delete-account__cancel span`).click((() => {
                    this.handleCancel();
                }));
            }
            getFieldConfigs() {
            ...
            async sendVerifyCode() {
                ...
                const {stoken: lastStoken} = await sendUniversalVerificationCode(params);
            ...
            async onSubmit(formValue = {}) {
                const params = this.UDBParams;
                const {stoken, data} = await verifyUniversalVerificationCode(params, {
                ...
                const {data: currentTime} = await getCurrentTime();
            ...
            async nextStep(currentTime) {
                ...
                __SL_$__(".delete-account__notice .submit-button").click((async e => {
                    ...
                    try {
                        ...
                        if (e.code === TIMEOUT_CODE) $error.show();
                    }
        ...
        class Account extends card {
            ...
            init() {
                this.$nickname = __SL_$__(`#${this.id} .account-item__nickname`);
                this.bindEvents();
            }
            bindEvents() {
                ...
                __SL_$__(`#${this.id} .account-item`).on("click", "a", (e => {
                    e.preventDefault();
                    const path = __SL_$__(e.currentTarget).data("path");
                    const type = __SL_$__(e.currentTarget).data("type");
                ...
                __SL_$__(`#${this.id} .account-item input`).on("input", (() => {
                    this.validateFields();
                }));
            }
            ...
            async openDeleteModal() {
                ...
                try {
                    const {data} = await getStoreRegisterConfig();
        ...
        const updateMemberInfo = data => request_request.post("/user/front/userinfo/updateMemberInfo", data);
        ...
        class Personal extends card {
            constructor({id, editable}) {
                ...
                const {birthday, gender} = SL_State.get("customer.userInfoDTO");
            ...
            init() {
                ...
                this.datepicker = new date_picker({
                    ...
                    onChange: date => {
                        ...
                        this.formData.birthday = dayjs_min_default()(date).format("YYYYMMDD");
                    }
                });
                __SL_$__(this.genderRadioSelector).on("change", (() => {
                    reportChooseGender();
                }));
            }
            getGender() {
                return __SL_$__(`${this.genderRadioSelector}:checked`).val();
            }
            onEdit() {
                reportEditPersion();
            }
            onCancel() {
                ...
                __SL_$__(`${this.genderRadioSelector}:checked`).removeProp("checked");
                __SL_$__(`${this.genderRadioSelector}[value=${this.originData.gender}]`).prop("checked", "checked");
            ...
            onSave() {
                ...
                return updateMemberInfo({
                    ...
                    if (this.formData.birthday) __SL_$__(this.birthdaySelector).text(dayjs_min_default()(this.formData.birthday).format("YYYY-MM-DD"));
                    if (GENDER_MAP[gender]) __SL_$__(this.genderSelector).text(GENDER_MAP[gender]);
        ...
        class Address extends card {
            ...
            bindEvents() {
                ...
                __SL_$__(`#${this.id} .address__btn--edit`).click((() => reportEditAddress()));
                __SL_$__(`#${this.id} .address__link`).click((() => reportClickNewAddress()));
                __SL_$__(`#${this.id} .address__btn--add`).click((() => reportClickNewAddress()));
            }
            bindRemoveAddress() {
                __SL_$__(`#${this.id} .address__list`).on("click", ".address__btn--remove", (async e => {
                    e.preventDefault();
                    const $target = __SL_$__(e.currentTarget);
                ...
                __SL_$__(`#${this.modal.modalId} .delete-address-confirm__footer`).on("click", ".btn", (async e => {
                    const $target = __SL_$__(e.currentTarget);
        ...
        class FacebookSdk {
            ...
            init(props = this.initParams) {
                const {lang: Lang = this.lang, ...rest} = props;
        ...
        class Messenger {
            ...
            init() {
                getSubscriptAuths({
                    ...
                    this.handleResponse = e => {
                        const {ref, event} = e;
                        if (ref === this.fbProps.userRef && "opt_in" === event) this.retryGetSubscriptAuth();
                        if (ref === this.fbProps.userRef && "clicked" === event) reportClickSubscribeMessage();
                    };
                    this.initFb();
                }));
            }
            ...
            renderHtml() {
                const $messengerPlugin = __SL_$__(MESSENGER_PLUGIN);
            ...
            retryGetSubscriptAuth(count = 1) {
                ...
                getSubscriptAuths({
                    ...
                    if (success && data.hasAuthorized) updateSubscriptions({
                        ...
                        this.$loading.hide();
                        this.rebuildPlugin();
                    })); else if (count <= MaxRetryCount) setTimeout((() => {
        ...
        class Line {
            ...
            initEvent() {
                this.$btn.on("click", (() => {
                    ...
                    const {channelId, state} = this.lineProps;
        ...
        class SubModal {
            constructor({id, type, onSuccess, onUnsub, onFieldValueChange}) {
                this.id = id;
                this.type = type;
                this.onSuccess = onSuccess;
                this.onUnsub = onUnsub;
                this.onFieldValueChange = onFieldValueChange;
                this.$input = __SL_$__(`#${id}-form .sl-input__inpEle`);
                this.$unSubBtn = __SL_$__(`#MpModal${id} .customer-center-sub-modal__cancel`);
                this.modal = null;
                this.form = null;
                this.userInfo = SL_State.get("customer.userInfoDTO");
        ...
        class UnSubModal {
            constructor({id, onSuccess, onCancel}) {
                this.id = id;
                this.type = null;
                this.onSuccess = onSuccess;
                this.onCancel = onCancel;
                this.modal = null;
                this.$cancelBtn = __SL_$__(`#MpModal${id} .customer-center-unsub-modal__btns--cancel`);
                this.$saveBtn = __SL_$__(`#MpModal${id} .customer-center-unsub-modal__btns--save`);
                this.init();
            }
            init() {
                const modal = new Modal({
            ...
            initModal() {
                this.unsubModal = new unsub_modal({
                    ...
                    onSuccess: () => {
                        ...
                        } else if ("line" === this.type) this.$subLinetem.removeAttr("data-subscribed"); else if ("messenger" === this.type) {
                        ...
                        if (typeMap[this.type]) reportUnsubscribeConfirm(typeMap[this.type]);
                    },
                    onCancel: () => {
        ...
        const account_BUTTON_LOADING_CLASS = "btn--loading";
        class Information {
            constructor() {
                this.init();
                this.personal = null;
