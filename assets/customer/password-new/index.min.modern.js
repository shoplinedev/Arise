(() => {
    ...
    (() => {
        ...
        const sendPhoneVerificationCode = params => request_udbRequest.get("/udb/aq/uni/sendSms.do", {
            params
        });
        const sendEmailVerificationCode = params => request_udbRequest.get("/udb/aq/uni/sendEmailCode.do", {
            params
        });
        const verifyPhoneVerificationCode = params => request_udbRequest.get("/udb/aq/uni/verifySms.do", {
            params
        });
        ...
        const getRetrievePrechk = params => request_udbRequest.get("/udb/aq/pwd/retrieve/prechk.do", {
            params
        });
        const getMethodList = params => request_udbRequest.get("/udb/aq/uni/getMethodList.do", {
            params
        });
        const resetPassword = params => request_udbRequest.get("/udb/aq/pwd/retrieve/modify.do", {
            params
        });
        ...
        const reports_reportV2 = collect => {
        ...
        const reportForgetPasswordToLogin = () => {
            reports_reportV2({
                page: report_pageMap.PasswordNew,
        ...
        const reportResetPasswordToLogin = () => {
            reports_reportV2({
                page: report_pageMap.PasswordNewReset,
                module: report_Module.normal,
                component: 101,
                action_type: report_ActionType.click,
                event_id: 1413
        ...
        const reportSignIn = config => reports_reportV2({
            page: report_pageMap.SignIn,
            ...config
        });
        ...
        const reportSignUp = config => reports_reportV2({
            page: report_pageMap.SignUp,
            ...config
        });
        ...
        const reportLoginModal = config => reports_reportV2({
            page: report_pageMap.LoginModal,
            ...config
        });
        const reportPageView = () => {
            const alias = window.SL_State.get("templateAlias");
            if (loginModalPageIdMap[alias]) reports_reportV2({
        ...
        class PasswordSet extends customer {
            ...
            init() {
                this.passwordForm = new commons_form({
                    id: this.formId,
                    fields: this.getFieldConfigs(),
                    onSubmit: data => this.onSubmit(data).then((res => Promise.resolve(res))).catch((error => {
        ...
        const password_set = PasswordSet;
        class PasswordNew extends customer {
            ...
            init() {
                ...
                __SL_$__(`#${this.containerId} .password__buttons a`).click((() => {
                    const display = __SL_$__(`#${this.formId}`).css("display");
                    if ("none" !== display) reportForgetPasswordToLogin(); else reportResetPasswordToLogin();
                }));
            }
            getFieldConfigs() {
            ...
            async sendVerifyCode() {
                ...
                await wrapArmorCaptcha({
                    beforeSendCode: async () => {
                        ...
                        const {data: {methods}, stoken: newStoken} = await getMethodList(super.formatRequestBody(UDBParams));
                        const {method, mobileMask, emailMask} = methods && methods[0] || {};
                        super.updateToken(UDBParams, {
                            stoken: newStoken,
                            _method: method,
            ...
            initSetForm() {
                __SL_$__(`#${this.formId}`).hide();
                const passwordSetForm = new password_set({
                    id: this.setFormId,
                ...
                __SL_$__(`#${passwordSetForm.formId}`).show();
            }
            onResetConfirm(data) {
            ...
            onSubmit() {
                const {UDBParams} = this;
                const {verifycode} = this.passwordForm && this.passwordForm.getFormValue();
                if (UDBParams.stoken) {
        ...
        const password_new = password_new_PasswordNew;
        __SL_$__((function() {
            if (!document.getElementById("customer-password")) return false;
            new password_new({
