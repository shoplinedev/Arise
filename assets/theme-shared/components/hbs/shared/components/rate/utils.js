...
window.SLM['theme-shared/components/hbs/shared/components/rate/utils.js'] = window.SLM['theme-shared/components/hbs/shared/components/rate/utils.js'] || function () {
  const _exports = {};
  ...
  _exports.checkPrecision = checkPrecision;
  ...
  _exports.checkBounds = checkBounds;
  ...
  const isNumber = input => isType(input, 'number');

  _exports.isNumber = isNumber;

  const isString = input => isType(input, 'string');

  _exports.isString = isString;

  const isFunction = input => isType(input, 'function');

  _exports.isFunction = isFunction;

  const isObject = input => isType(input, 'object');
  ...
  function each(input, fn) {
    if (Array.isArray(input)) {
      return input.forEach((item, index) => fn.call(item, item, index));
    }
