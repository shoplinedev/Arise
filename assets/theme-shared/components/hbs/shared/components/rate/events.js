...
window.SLM['theme-shared/components/hbs/shared/components/rate/events.js'] = window.SLM['theme-shared/components/hbs/shared/components/rate/events.js'] || function () {
  const _exports = {};
  const { isFunction, isDefined } = window['SLM']['theme-shared/components/hbs/shared/components/rate/utils.js'];
  ...
  const Event = isFunction(window.Event) ? window.Event : function Event(event, params = {}) {
    const {
      bubbles = false,
      cancelable = false
    } = params;
  ...
  const CustomEvent = isFunction(window.CustomEvent) ? window.CustomEvent : (function CustomEvent(event, params = {}) {
    const {
      bubbles = false,
      cancelable = false,
      detail = void 0
    ...
    return evt;
  }, CustomEvent.prototype = Object.create(Event.prototype), CustomEvent);
  _exports.default = {
